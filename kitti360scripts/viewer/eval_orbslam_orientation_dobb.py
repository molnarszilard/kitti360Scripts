...
parser.add_argument('--list', default='/mnt/ssd2/datasets/kitti360_pose3_veh_build/2013_05_28_drive_0009_sync/frames_class_both.txt',  #'/mnt/ssd2/datasets/kitti360_pose2_veh_build/2013_05_28_drive_0010_sync/frames_class_both.txt'
...
orbslam_results_path=os.path.join(args.kitti_root,'orbslam_results',args.sequence+'.txt')
...
def process():
    ...
    for frame in frames2check:
        if len(orbslam_results)>frame:
            orbslam_estimate = orbslam_results[frame]
        else:
            break
        ...
        error_orb = abs(math.atan2(math.sin(ang_diff_orbslam - ang_diff_camera),math.cos(ang_diff_orbslam - ang_diff_camera)))
        error_dobb = abs(math.atan2(math.sin(ang_diff_dobb - ang_diff_camera),math.cos(ang_diff_dobb - ang_diff_camera)))

        print('Frame: %d, -> orb: %f, cam: %f, dobb: %f'%(frame,math.degrees(ang_diff_orbslam),math.degrees(ang_diff_camera),math.degrees(ang_diff_dobb)))
        row.append(math.degrees(ang_diff_orbslam))
        row.append(math.degrees(ang_diff_camera))
        row.append(math.degrees(ang_diff_dobb))
        row.append(math.degrees(error_orb))
