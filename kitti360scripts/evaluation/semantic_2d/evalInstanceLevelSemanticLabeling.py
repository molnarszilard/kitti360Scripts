...
#   {seq:0>4}_{frame:0>10}*.txt
# for a ground truth filename
#   2013_05_28_drive_{seq:0>4}_sync/image_00/instance/{frame:0>10}.png
def getPrediction( args, groundTruthFile ):
    ...
    filePattern = "{:04d}_{:010d}*.txt".format( csFile.sequenceNb , csFile.frameNb )
...
def setInstanceLabels(args):
    ...
    for label in labels:
        if label.name in ['train', 'bus']:
            continue
        if label.hasInstances and not label.ignoreInInst:
...
# Routine to read ground truth confidence normalized to [0,1]
def readGTConf(gtConfFileName,args):
    conf = Image.open(gtConfFileName)
    return np.array(conf) / MAX_CONFIDENCE
...
def assignGt2Preds(gtInstancesOrig, gtImage, gtConf, predInfo, args):
    ...
    for predImageFile in predInfo:
        ...
        # pixel count weighted by ground truth confidence
        weightedPredPixelCount = np.sum( gtConf[boolPredInst] )

        # skip if actually empty
        if np.isclose(weightedPredPixelCount, 0):
            continue
        ...
        for (gtNum,gtInstance) in enumerate(gtInstancesOrig[labelName]):
            ...
            # intersection count weighted by ground truth confidence
            intersectionMask = np.logical_and( gtNp == gtInstance["instID"] , boolPredInst)
            intersection     = np.sum(gtConf[intersectionMask])
